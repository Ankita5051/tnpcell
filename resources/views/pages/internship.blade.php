...
<div class="job-banner">
    <div class="job-para">
       <h1 class="common-heading">internship search</h1>
       <p class="banner-para">
       Find a internship you'll love fast with expert advice from recruitment experts. Follow step-by-step instructions and master the job search.
       </p>
    </div>
        ...
        <h4><i class="fa-regular fa-building"></i>  {{$job->company_name}}</h4>
         ...
         <div class="job-info">
            ...
            <h4><i class="fa-solid fa-briefcase"></i>  
                ...
                {{$job->type}}</h4>
            <h4><i class="fa-regular fa-calendar-xmark"></i> 
                @php
$s = $job->deadline;
$dt = new DateTime($s);
//print_r ($dt);
...
{{$date}}
         ...
         <div class="expiry">
            <a href="{{$job->job_link}}" class="btn btn-primary common-btn">know more</a>
...
$("#filter-btn").on('click',function(e){
    ...
    function jobDetail(e,id){
        e.preventDefault();
// item=$(e.target);
//         $(this).css('box-shadow','inset 0px 0px 5px 0px');
//         item.css('border','1px solid #a8a4fd');
        $.ajax({
            url:'/jobdetail/'+id,
            type:'GET',
            success:function(res){
